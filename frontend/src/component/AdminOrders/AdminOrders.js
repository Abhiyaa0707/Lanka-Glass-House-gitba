...
function AdminOrders() {
  ...
  const handlOrder = (orderIndex) => {
    ...
    alert('Order has been Deliverd Successfully.');
  };

  const downloadReport = () => {
    const doc = new jsPDF();
    const headers = ["Order Number", "Customer Email", "Delivery Address", "Payment (Card Number)", "Amount", "Ordered At"];
    const data = orders.map(order => [
      order.orderNumber,
      order.email,
      order.deliveryAddress,
      order.cardNumber.replace(/\d(?=\d{4})/g, "*"),
          ...
          <tbody>
            {filteredOrders.map((order, index) => (
              <tr key={index}>
                ...
                <td>{order.cardNumber.replace(/\d(?=\d{4})/g, "*")}</td>
